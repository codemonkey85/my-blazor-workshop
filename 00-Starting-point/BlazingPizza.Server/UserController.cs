...
namespace BlazingPizza.Server
{
    ...
    {
        private static readonly UserState LoggedOutState = new UserState { IsLoggedIn = false };

        [HttpGet("user")]
        public UserState GetUser() => User.Identity.IsAuthenticated
                ? new UserState { IsLoggedIn = true, DisplayName = User.Identity.Name }
                : LoggedOutState;

        [HttpGet("user/signin")]
        public async Task SignIn() => await HttpContext.ChallengeAsync(
